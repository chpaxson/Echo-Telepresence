...
static void twai_receive_task(void *arg)
{
    while (1) {
        ...
        // Check if a new action is received. Use a small timeout to allow checking for messages periodically.
        if (xQueueReceive(rx_task_queue, &action, pdMS_TO_TICKS(10)) == pdPASS) {
            // Handle task exit action immediately
            if (action == RX_TASK_EXIT) {
                break;
            }
        }

        // Always try to receive a message, regardless of the current action state
        twai_message_t rx_msg;
        esp_err_t ret = twai_receive(&rx_msg, pdMS_TO_TICKS(50)); // Use a timeout to avoid blocking indefinitely if no message arrives

        if (ret == ESP_OK) {
            // Log every received message
            ESP_LOGI(EXAMPLE_TAG, "Received msg ID: 0x%03"PRIx32", DLC: %d, Data:", rx_msg.identifier, rx_msg.data_length_code);
            // Consider adding data logging here if needed, e.g., hex dump
            // Example: Log first few bytes
            // char data_str[25] = {0}; // Max 8 bytes * 3 chars ("XX ") + null terminator
            // for (int i = 0; i < rx_msg.data_length_code && i < 8; i++) {
            //     snprintf(data_str + i * 3, 4, "%02X ", rx_msg.data[i]);
            // }
            // ESP_LOGI(EXAMPLE_TAG, "  Data: %s", data_str);


            // Process the received message based on the current expected action
            if (action == RX_RECEIVE_PING_RESP) {
                if (rx_msg.identifier == ID_SLAVE_PING_RESP) {
                    ESP_LOGI(EXAMPLE_TAG, "Ping response received");
                    xSemaphoreGive(stop_ping_sem);
                    xSemaphoreGive(ctrl_task_sem);
                    // Reset action after handling to wait for the next command
                    action = (rx_task_action_t)0; // Or some idle/invalid state
                }
            } else if (action == RX_RECEIVE_DATA) {
                // This static variable is potentially problematic if the task logic changes significantly.
                // Consider passing expected count via queue or using a different state management approach.
                static uint32_t data_msgs_rec = 0;
                if (rx_msg.identifier == ID_SLAVE_DATA) {
                    uint32_t data = 0;
                    // Check DLC before accessing data
                    if (rx_msg.data_length_code <= sizeof(data)) {
                         for (int i = 0; i < rx_msg.data_length_code; i++) {
                            data |= (rx_msg.data[i] << (i * 8));
                        }
                        ESP_LOGI(EXAMPLE_TAG, "Received data value %"PRIu32, data);
                    } else {
                         ESP_LOGW(EXAMPLE_TAG, "Received data msg with unexpected DLC: %d", rx_msg.data_length_code);
                    }
                    data_msgs_rec++;
                    if (data_msgs_rec >= NO_OF_DATA_MSGS) {
                        ESP_LOGI(EXAMPLE_TAG, "All data messages received");
                        xSemaphoreGive(ctrl_task_sem);
                        data_msgs_rec = 0; // Reset counter for next iteration
                        // Reset action after handling
                        action = (rx_task_action_t)0;
                    }
                }
            } else if (action == RX_RECEIVE_STOP_RESP) {
                if (rx_msg.identifier == ID_SLAVE_STOP_RESP) {
                    ESP_LOGI(EXAMPLE_TAG, "Stop response received");
                    xSemaphoreGive(ctrl_task_sem);
                    // Reset action after handling
                    action = (rx_task_action_t)0;
                }
            ...
            ESP_LOGE(EXAMPLE_TAG, "TWAI receive error: %s", esp_err_to_name(ret));
        }
        // Small delay to prevent high CPU usage if continuously receiving non-matching messages or timeouts
