...
void  StepperDriver4PWM::enable(){
    ...
    if ( _isset(enable_pin1) ) gpio_put(enable_pin1, enable_active_high);
    if ( _isset(enable_pin2) ) gpio_put(enable_pin2, enable_active_high);
    // set zero to PWM
    setPwm(0,0);
}
...
int StepperDriver4PWM::init() {
    ...
    // PWM pins
    gpio_init(pwm1A); gpio_set_dir(pwm1A, GPIO_OUT);
    gpio_init(pwm1B); gpio_set_dir(pwm1B, GPIO_OUT);
    gpio_init(pwm2A); gpio_set_dir(pwm2A, GPIO_OUT);
    gpio_init(pwm2B); gpio_set_dir(pwm2B, GPIO_OUT);

    if (_isset(enable_pin1)) {
        gpio_init(enable_pin1); gpio_set_dir(enable_pin1, GPIO_OUT);
    }
    if (_isset(enable_pin2)) {
        gpio_init(enable_pin2); gpio_set_dir(enable_pin2, GPIO_OUT);
    }
...
void StepperDriver4PWM::setPhaseState(PhaseState sa, PhaseState sb) {
    ...
    if( _isset(enable_pin1) &&  _isset(enable_pin2)){
        gpio_put(enable_pin1, sa == PhaseState::PHASE_ON ? enable_active_high:!enable_active_high);
        gpio_put(enable_pin2, sb == PhaseState::PHASE_ON ? enable_active_high:!enable_active_high);
    }
}
